# soominhyunwoovisor Quick Start

`cosmovisor` is a small process manager around soominhyunwoo SDK binaries that monitors the governance module via stdout to see if there's a chain upgrade proposal coming in. If it see a proposal that gets approved it can be run manually or automatically to download the new code, stop the node, run the migration script, replace the node binary, and start with the new genesis file.

## Installation

Run:

`go get github.com/soominhyunwoo/chain-sdk/cosmovisor/cmd/cosmovisor`
...
Each version of the soominhyunwoo SDK application is stored under either `genesis` or `upgrades/<name>`, which holds `bin/$DAEMON_NAME`
...
shipped along the soominhyunwoo SDK's source code.

First compile `simd`:

```
cd soominhyunwoo-sdk/
make build
```
...
For the sake of this demonstration, we would amend `voting_params.voting_period` in `.simapp/config/genesis.json` to a reduced time ~5 minutes (300s) and eventually launch `soominhyunwoovisor`:
...
After that line, add the following snippet:
 ...
 app.UpgradeKeeper.SetUpgradeHandler("test1", func(ctx sdk.Context, plan upgradetypes.Plan) {
		...
		addr, err := sdk.AccAddressFromBech32("soominhyunwoo18cgkqduwuh253twzmhedesw3l7v3fm37sppt58")
		if err != nil {
			panic(err)
		}
...
Now recompile a new binary and place it in `$DAEMON_HOME/soominhyunwoovisor/upgrades/test1/bin`:
