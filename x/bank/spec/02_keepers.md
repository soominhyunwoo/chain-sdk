...
By providing the `x/bank` module with a blocklisted set of addresses, an error occurs for the operation if a user or client attempts to directly or indirectly send funds to a blocklisted account, for example, by using [IBC](http://docs.soominhyunwoo.network/master/ibc/).
...
message Input {
  string   address                        = 1;
  repeated soominhyunwoo.base.v1beta1.Coin coins = 2;
}
```
