...
import (
	...
	tmcli "github.com/soominhyunwoo/tendermint/libs/cli"
	...
	"github.com/soominhyunwoo/chain-sdk/x/feegrant/client/cli"
...
func (s *IntegrationTestSuite) TestCmdGetFeeGrant() {
	...
	testCases := []struct {
		...
		},
		{
			"non existed grant",
			[]string{
				"soominhyunwoo1nph3cfzk6trsmfxkeu943nvach5qw4vwstnvkl",
				grantee.String(),
				fmt.Sprintf("--%s=json", tmcli.OutputFlag),
			},
			"fee-grant not found",
	...
	testCases := []struct {
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo17h5lzptx3ghvsuhk7wx4c4hnl7rsswxjer97em",
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
				commonFlags...,
			),
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo16dlc38dcqt0uralyd8hksxyrny6kaeqfjvjwp5",
					fmt.Sprintf("--%s=%s", cli.FlagSpendLimit, "100stake"),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
				commonFlags...,
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo1ku40qup9vwag4wtf8cls9mkszxfthaklxkp3c8",
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
				commonFlags...,
			),
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo1w55kgcf3ltaqdy4ww49nge3klxmrdavrr6frmp",
					fmt.Sprintf("--%s=%s", cli.FlagSpendLimit, "100stake"),
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo1vevyks8pthkscvgazc97qyfjt40m6g9xe85ry8",
					fmt.Sprintf("--%s=%d", cli.FlagPeriod, oneHour),
					fmt.Sprintf("--%s=%s", cli.FlagPeriodLimit, "10stake"),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
					fmt.Sprintf("--%s=%s", cli.FlagExpiration, getFormattedExpiration(tenHours)),
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo14cm33pvnrv2497tyt8sp9yavhmw83nwej3m0e8",
					fmt.Sprintf("--%s=%s", cli.FlagSpendLimit, "100stake"),
					fmt.Sprintf("--%s=%d", cli.FlagPeriod, oneHour),
					fmt.Sprintf("--%s=%s", cli.FlagPeriodLimit, "10stake"),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo12nyk4pcf4arshznkpz882e4l4ts0lt0ap8ce54",
					fmt.Sprintf("--%s=%d", cli.FlagPeriod, oneHour),
					fmt.Sprintf("--%s=%s", cli.FlagPeriodLimit, "10stake"),
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
	...
	testCases := []struct {
		...
		},
		{
			...
			append(
				[]string{
					granter.String(),
					"soominhyunwoo1aeuqja06474dfrj7uqsvukm6rael982kk89mqr",
					fmt.Sprintf("--%s=%s", flags.FlagFrom, granter),
				},
				commonFlags...,
			),
