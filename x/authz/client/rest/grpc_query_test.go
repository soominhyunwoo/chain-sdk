...
import (
	...
	authztestutil "github.com/soominhyunwoo/chain-sdk/x/authz/client/testutil"
...
func (s *IntegrationTestSuite) TestQueryGrantGRPC() {
	val := s.network.Validators[0]
	grantsURL := val.APIAddress + "/soominhyunwoo/authz/v1beta1/grants?granter=%s&grantee=%s&msg_type_url=%s"
	testCases := []struct {
		name      string
		url       string
		expectErr bool
...
func (s *IntegrationTestSuite) TestQueryGrantsGRPC() {
	val := s.network.Validators[0]
	grantsURL := val.APIAddress + "/soominhyunwoo/authz/v1beta1/grants?granter=%s&grantee=%s"
	testCases := []struct {
		name      string
		url       string
		expectErr bool
