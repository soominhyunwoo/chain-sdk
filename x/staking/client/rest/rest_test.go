...
func (s *IntegrationTestSuite) TestLegacyGetValidators() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			true, "soominhyunwoo sdk v0.40 introduces a breaking change on this endpoint: instead of" +
				" querying using `?status=bonded`, please use `status=BOND_STATUS_BONDED`. For more" +
				" info, please see our REST endpoint migration guide at https://docs.soominhyunwoo.network/master/migrations/rest.html",
		},
		{
