...
import (
	...
	clienttx "github.com/soominhyunwoo/chain-sdk/client/tx"
	"github.com/soominhyunwoo/chain-sdk/crypto/hd"
	"github.com/soominhyunwoo/chain-sdk/crypto/keyring"
	kmultisig "github.com/soominhyunwoo/chain-sdk/crypto/keys/multisig"
	...
	"github.com/soominhyunwoo/chain-sdk/types/tx/signing"
	authclient "github.com/soominhyunwoo/chain-sdk/x/auth/client"
	authtest "github.com/soominhyunwoo/chain-sdk/x/auth/client/testutil"
	bankcli "github.com/soominhyunwoo/chain-sdk/x/bank/client/testutil"
...
func (s IntegrationTestSuite) TestSimulateTx_GRPCGateway() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			s.Require().NoError(err)
			res, err := rest.PostRequest(fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/simulate", val.APIAddress), "application/json", req)
			s.Require().NoError(err)
			if tc.expErr {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expErr {
				...
				// ref: https://github.com/soominhyunwoo/chain-sdk/issues/8680
				// ref: https://github.com/soominhyunwoo/chain-sdk/issues/8681
...
func (s IntegrationTestSuite) TestGetTxEvents_GRPCGateway() {
	...
	testCases := []struct {
		...
		{
			"empty params",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs", val.APIAddress),
			true,
			"must declare at least one event to search",
		},
		{
			"without pagination",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s", val.APIAddress, bankMsgSendEventAction),
			false,
			"",
		},
		{
			"with pagination",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s&pagination.offset=%d&pagination.limit=%d", val.APIAddress, bankMsgSendEventAction, 0, 10),
			false,
			"",
		},
		{
			"valid request: order by asc",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s&events=%s&order_by=ORDER_BY_ASC", val.APIAddress, bankMsgSendEventAction, "message.module='bank'"),
			false,
			"",
		},
		{
			"valid request: order by desc",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s&events=%s&order_by=ORDER_BY_DESC", val.APIAddress, bankMsgSendEventAction, "message.module='bank'"),
			false,
			"",
		},
		{
			"invalid request: invalid order by",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s&events=%s&order_by=invalid_order", val.APIAddress, bankMsgSendEventAction, "message.module='bank'"),
			true,
			"is not a valid tx.OrderBy",
		},
		{
			"expect pass with multiple-events",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s&events=%s", val.APIAddress, bankMsgSendEventAction, "message.module='bank'"),
			false,
			"",
		},
		{
			"expect pass with escape event",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs?events=%s", val.APIAddress, "message.action%3D'/soominhyunwoo.bank.v1beta1.MsgSend'"),
			false,
			"",
		},
	}
...
func (s IntegrationTestSuite) TestGetTx_GRPCGateway() {
	...
	testCases := []struct {
		...
		{
			"dummy hash",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs/%s", val.APIAddress, "deadbeef"),
			true, "tx (DEADBEEF) not found",
		},
		{
			"good hash",
			fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs/%s", val.APIAddress, s.txRes.TxHash),
			false, "",
		},
	}
	for _, tc := range testCases {
...
func (s IntegrationTestSuite) TestBroadcastTx_GRPCGateway() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			req, err := val.ClientCtx.JSONCodec.MarshalJSON(tc.req)
			s.Require().NoError(err)
			res, err := rest.PostRequest(fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs", val.APIAddress), "application/json", req)
			s.Require().NoError(err)
			if tc.expErr {
				s.Require().Contains(string(res), tc.expErrMsg)
			} else {
