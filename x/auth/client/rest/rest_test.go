...
import (
	...
	"github.com/soominhyunwoo/chain-sdk/client/tx"
	...
	authcli "github.com/soominhyunwoo/chain-sdk/x/auth/client/cli"
...
func (s *IntegrationTestSuite) TestQueryAccountWithColon() {
	...
	// colon don't 501. See
	// https://github.com/soominhyunwoo/chain-sdk/issues/8650
	addrWithColon := "soominhyunwoo:1m4f6lwd9eh8e5nxt0h00d46d3fr03apfh8qf4g"

	res, err := rest.GetRequest(fmt.Sprintf("%s/soominhyunwoo/auth/v1beta1/accounts/%s", val.APIAddress, addrWithColon))
	s.Require().NoError(err)
...
func (s *IntegrationTestSuite) testQueryIBCTx(txRes sdk.TxResponse, cmd *cobra.Command, args []string) {
	...
	grpcJSON, err := rest.GetRequest(fmt.Sprintf("%s/soominhyunwoo/tx/v1beta1/txs/%s", val.APIAddress, txRes.TxHash))
	s.Require().NoError(err)
...
// ref: https://github.com/soominhyunwoo/chain-sdk/issues/8679
